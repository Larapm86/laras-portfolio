...
const nodes = [
  {
    ...
    shape: 'starburst', color: '#d04e28', r: 14,
  ...
  },
  {
    ...
    shape: 'circle-fill', color: '#1a1814', r: 8,
  ...
  },
  {
    ...
    shape: 'circle-fill', color: '#1a1814', r: 7,
  ...
  },
  {
    ...
    shape: 'hexagon', color: '#1a1814', r: 10,
  ...
  },
  {
    ...
    shape: 'triangle', color: '#1a1814', r: 9,
...
function worldPos(node) {
  ...
  return {
    ...
    y: (base.y * H - 48) * vp.scale + vp.y + 48
  }
}
...
function drawShape(shape, x, y, r, color, hovered) {
  ...
  if (shape === 'hexagon') {
    ...
    ctx.stroke()
    if (hovered) { ctx.fillStyle = 'rgba(26,24,20,0.06)'; ctx.fill() }
  } else if (shape === 'circle-fill') {
    ctx.fillStyle = color
    ctx.fill()
...
function draw() {
  ...
  edges.forEach(e => {
    ...
    ctx.strokeStyle = isHighlighted ? 'rgba(26,24,20,0.45)' : edgeHighlighted ? 'rgba(26,24,20,0.15)' : 'rgba(26,24,20,0.08)'
  ...
  nodes.forEach(n => {
    ...
    ctx.font = `${hovered || legendHighlight ? '500' : '400'} ${hovered ? 13 : 12}px 'DM Mono', monospace`
    lines.forEach((line, i) => {
      ctx.fillText(line, pos.x, pos.y + offsetY + i * lineHeight)
    })
    if (n.sub && n.id !== 'lara') {
      ctx.font = `italic 400 11px 'Cormorant Garamond', serif`
...
function updateCenterLabel() {
  ...
  // Place label below the starburst (r=14) so "Lara Perez" doesn’t sit on the red shape
...
function focusOnNode(node) {
  ...
  vp.y = H / 2 - 48 - (base.y * H - 48) * vp.scale - 48
  draw()
  updateCenterLabel()
...
export function initGraph() {
  ...
  canvas.addEventListener('wheel', e => {
    ...
    vp.x = mx - (mx - vp.x) * factor
    vp.y = my - (my - vp.y) * factor
    vp.scale *= factor
    vp.scale = Math.max(0.4, Math.min(2.5, vp.scale))
    draw()
    updateCenterLabel()
  }, { passive: false })
  ...
  const moreInfoBtn = document.getElementById('graph-more-info-btn')
  const moreInfoContent = document.getElementById('graph-more-info-content')
  if (moreInfoBtn && moreInfoContent) {
    moreInfoBtn.addEventListener('click', (e) => {
      e.preventDefault()
      e.stopPropagation()
      const isOpen = moreInfoContent.hidden
      moreInfoContent.hidden = !isOpen
      moreInfoBtn.setAttribute('aria-expanded', isOpen ? 'true' : 'false')
      moreInfoBtn.textContent = isOpen ? 'less' : 'more information'
    })
  }
